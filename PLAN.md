...
│  ┌───────────────────────┴───────────────────────────────┐   │
│  │            💬 Chat Interface                           │   │
│  │   NL commands → agent → pipeline → results            │   │
│  └───────────────────────┬───────────────────────────────┘   │
└──────────────────────────┼───────────────────────────────────┘
                           │ API
┌──────────────────────────┼───────────────────────────────────┐
│              AGENT LAYER (Claude Haiku via tool use)          │
│                          │                                   │
│  ┌───────────────────────▼──────────────────────────────┐    │
│  │  NL → Filters: parse natural language into config    │    │
│  │  Refinement: re-run pipeline with adjusted params    │    │
│  │  Descriptions: narrate results in natural language   │    │
│  │  Style Interpretation: understand reference intent   │    │
│  │  Multi-Location: plan itinerary across areas         │    │
│  └──────────────────────┬───────────────────────────────┘    │
│                          │ tool calls                        │
└──────────────────────────┼───────────────────────────────────┘
                           │
...
│  └──────────────────────┬───────────────────────────────┘    │
│  ┌──────────────────────▼───────────────────────────────┐    │
│  │  Phase 7: Optimal Lighting Time                      │    │
│  │  For each viewpoint, sample sun positions through     │    │
│  │  the day → cast shadows on DEM (reuses viewshed) →   │    │
│  │  score shadow/light ratio, rake angle, peak-lit/      │    │
│  │  valley-shadow contrast → output best time of day    │    │
│  └──────────────────────┬───────────────────────────────┘    │
│  ┌──────────────────────▼───────────────────────────────┐    │
│  │  Phase 8: Enhancement + Export                        │    │
│  │  Nano Banana 2 → photorealistic preview              │    │
│  │  Style reference → IP-Adapter + ControlNet           │    │
│  │  Drone export → Litchi CSV with GPS/alt/heading/pitch│    │
│  └──────────────────────────────────────────────────────┘    │
└──────────────────────────────────────────────────────────────┘
```
...
For each final viewpoint, compute the best time of day for dramatic light. The sun is just a directional light source, and shadow casting on a DEM is the same ray-casting we already use for viewshed analysis.

**How it works:**

1. Compute sun azimuth + elevation every 15 minutes from sunrise to sunset (~50 samples) using `astral` or `pvlib`
2. For each sun position, cast shadows on the DEM — for each terrain cell, trace a ray toward the sun; if it hits higher terrain, that cell is in shadow. This is viewshed from the sun's direction.
3. Score the shadow pattern within the camera's visible area:

```python
for t in sample_times(sunrise, sunset, step=15min):
    sun_az, sun_el = solar_position(lat, lng, t)
    shadow_map = cast_shadows(dem, sun_az, sun_el)

    visible = viewshed(dem, camera_pos)
    shadow_ratio = (visible & shadow_map).sum() / visible.sum()

    scores[t] = (
        0.30 * gaussian(shadow_ratio, center=0.35, sigma=0.15) +  # 35% shadow is ideal
        0.25 * gaussian(sun_el, center=8, sigma=10) +              # low sun = dramatic
        0.25 * rake_score(sun_az, cliff_normals) +                 # light perpendicular to cliffs
        0.20 * peak_lit_valley_shadow(shadow_map, dem, visible)    # classic light pattern
    )
```

**Shadow scoring factors:**

| Factor | Why | Optimal |
|---|---|---|
| Shadow/light ratio | Too little shadow = flat; too much = dark | ~35% shadow |
| Sun elevation angle | Low sun creates long, dramatic shadows | ~8° above horizon (golden hour) |
| Rake angle | Light perpendicular to cliff faces reveals texture and depth | 90° to surface normal |
| Peak-lit / valley-shadow | The classic landscape look — lit summits, shadowed valleys | High contrast |

**Output per viewpoint:**
- Best time: "7:14 AM" (or "6:42 PM")
- Lighting description: "Low-angle eastern light rakes across the cliff face, peaks lit with valley in shadow"
- Secondary time: "5:58 PM"
- Mini timeline of scores throughout the day

The best achievable shadow score also feeds back into the overall viewpoint ranking — a compositionally perfect viewpoint that never gets dramatic light ranks lower than one that does.

**Time estimate:** ~2-3 seconds per viewpoint (50 shadow casts × ~50ms each, reusing viewshed infrastructure).
...
**Drone flight path export:** Export top viewpoints as a Litchi-compatible CSV for automated drone missions:

```csv
latitude,longitude,altitude(m),heading(deg),curvesize(m),rotationdir(0),gimbalmode(2),gimbalpitchangle(deg),actiontype1(1),actionparam1(0),altitudemode(1)
37.7459,-119.5332,150,245,0,0,2,-15,1,0,1
37.7512,-119.5401,200,180,0,0,2,-25,1,0,1
```

Columns map directly from our computed camera poses: lat/lng → position, altitude → camera height (MSL), heading → yaw, gimbalpitchangle → pitch. Action type 1 = take photo.

Viewpoints are ordered by a simple nearest-neighbor TSP solver to minimize flight distance. The user imports the CSV into Litchi, flies the mission, and the drone automatically navigates to each viewpoint, sets up the exact shot, and triggers the shutter.
...
The agent wraps the pipeline — it doesn't sit inside it. The core math (terrain analysis, inverse PnP, scoring, CMA-ES) runs at native speed with zero LLM overhead. The agent orchestrates these phases via tool calls and adds intelligence at the boundaries where natural language understanding matters.

### Why an Agent (and where it doesn't slow things down)

The pipeline's compute-heavy phases (terrain analysis, camera computation, beauty scoring, CMA-ES) are pure math — NumPy, SciPy, linear algebra. An LLM in the loop there would add latency for zero benefit. The agent adds value at five specific touchpoints where structured computation can't handle ambiguity:

### Agent Capabilities

#### 1. Natural Language → Filter Configuration

User says: *"Find dramatic cliff shots near Yosemite, something with depth and a strong foreground"*

The agent translates this to pipeline parameters:

```python
# Agent interprets NL → structured config
{
    "center": {"lat": 37.7459, "lng": -119.5332},  # "near Yosemite"
    "radius_km": 15,
    "feature_weights": {
        "cliffs": 0.9,      # "dramatic cliff shots"
        "relief": 0.8,      # "dramatic"
        "ridges": 0.5
    },
    "beauty_weights": {
        "depth_layering": 0.9,   # "depth"
        "prospect_refuge": 0.7   # "strong foreground" → foreground enclosure
    },
    "composition_prefs": ["leading_line", "rule_of_thirds"],
    "mode": "ground"
}
```

This replaces manual slider adjustment. The user describes what they want in words; the agent configures the 15+ parameters.

#### 2. Result Refinement Loop

After results come back, the user can say: *"These are too far from the cliff edge. I want to feel like I'm right on the edge looking down."*

The agent interprets this as adjustments and re-runs the pipeline:

```python
# Agent adjusts and re-runs
tools.run_pipeline({
    ...previous_config,
    "min_slope_at_camera": 30,         # camera on steep terrain
    "max_distance_from_cliff": 50,      # close to edge
    "pitch_preference": "downward",     # looking down
    "beauty_weights": {
        "prospect_refuge": {"prospect": 0.9, "refuge": 0.3}  # open, exposed
    }
})
```

The agent can run multiple refinement cycles, each taking 30-60 seconds through the full pipeline. It maintains conversation context so each refinement builds on the last.

#### 3. Result Descriptions

For each viewpoint, the agent generates a photographer-friendly narrative:

> *"Viewpoint #3 — You're standing on a granite ledge at 2,847m, 40m back from the cliff edge. Half Dome fills the upper-right third of the frame with El Capitan's face in the left midground. A natural leading line runs from the pine tree cluster at your feet toward the valley floor. Best light at 6:42 AM when low eastern sun rakes across the cliff face, lighting the summit while the valley stays in shadow. Fractal complexity peaks here because the ridgeline nesting of Cathedral Rocks creates visual depth at exactly the right scale."*

This turns score breakdowns into actionable shooting notes. Generated in parallel for all results — doesn't block the pipeline.

#### 4. Style Reference Interpretation

When a user uploads a reference image, the agent adds semantic understanding on top of the geometric fingerprinting:

```
User uploads: [Ansel Adams photo of Half Dome]
Agent: "I see a dramatic black-and-white landscape with a single dominant
       peak filling the upper frame, strong tonal contrast between lit rock
       and shadowed valley, and a dark foreground anchoring the composition.
       I'll search for: isolated prominent peaks with steep east-facing
       slopes (for similar light/shadow contrast), clear skylines, and
       terrain that provides dark foreground mass."
```

The agent augments the contour topology matcher with high-level compositional intent: not just "find terrain with similar curves" but "find terrain that would produce similar dramatic impact."

#### 5. Multi-Location Itinerary Planning

For a photography trip spanning multiple areas:

```
User: "I'm spending 3 days in the Swiss Alps. Plan me a route hitting
       the best spots with optimal timing."

Agent:
  Day 1 — Lauterbrunnen Valley
    5:45 AM  Viewpoint at Männlichen ridge (golden hour, Eiger north face)
    7:30 AM  Move to Kleine Scheidegg (morning side-light on Jungfrau)
    5:15 PM  Stechelberg cliff edge (sunset rake angle on valley walls)

  Day 2 — Zermatt
    6:00 AM  Gornergrat viewpoint (Matterhorn dawn, classic composition)
    ...

  Route: 127km total driving, viewpoints ordered by time-of-day constraints.
  Litchi CSV for drone shots attached.
```

The agent chains multiple pipeline runs across different coordinates, cross-references optimal lighting times, and plans logistics.

### Architecture

```
User ↔ Chat UI ↔ Agent (Claude Haiku)
                     │
                     ├── tool: parse_location(text) → lat/lng
                     ├── tool: run_pipeline(config) → results
                     ├── tool: adjust_filters(adjustments) → results
                     ├── tool: describe_viewpoint(viewpoint) → narrative
                     ├── tool: interpret_reference(image) → augmented_config
                     └── tool: plan_itinerary(locations[], days) → schedule
```

The agent is stateless between turns (Claude Haiku via API). Conversation history is maintained by the frontend. Each tool call triggers one or more backend endpoints. Pipeline results are returned as structured JSON that the agent narrates.

**Latency budget:** Agent adds ~500ms per turn (Haiku inference). Pipeline runs are 30-60 seconds. The agent doesn't add meaningful overhead — it adds intelligence at the margins.

### Demo Flow

The killer demo for a hackathon:

1. User types: *"Show me the most dramatic cliffside shots within 20km of Yosemite Valley, I want that Ansel Adams feel"*
2. Agent: *"Searching for dramatic cliff formations near Yosemite with strong tonal contrast compositions..."* (starts pipeline)
3. 45 seconds later: results appear on the globe with fly-to animations
4. Agent describes the top 3 viewpoints in photographer language
5. User: *"I like #2 but can you find something with more foreground interest?"*
6. Agent adjusts depth_layering weight, re-runs → refined results in 30s
7. User: *"Perfect. When should I be there and export it to my drone"*
8. Agent: *"Best light at 6:42 AM. Litchi CSV downloaded — 3 waypoints, 4.2km flight path."*
...
| Agent / Chat | Claude Haiku (Anthropic API) | Fast, cheap (~$0.001/turn), tool use support |
...
- Grid of top results, each showing:
  ...
  - **Best time of day** with lighting description ("7:14 AM — eastern light rakes cliff face")
  - Mini timeline showing shadow quality score throughout the day
  ...
  - "Export to Drone" → download Litchi CSV for automated mission
...
- [ ] Chat interface component (text input + message history)
...
### Hour 4: Rendering + Lighting + Neural Scoring (3:00 - 4:00)

- [ ] CesiumJS headless rendering via Puppeteer
- [ ] LAION Aesthetics v2.5 integration
- [ ] Optimal lighting time computation (sun position + DEM shadow casting)
- [ ] CMA-ES refinement on top candidates
- [ ] Results gallery UI with score breakdowns + best time of day
- [ ] Drone export: Litchi CSV with GPS/altitude/heading/pitch

### Hour 5: Style Reference + Agent + Enhancement + Polish (4:00 - 5:00)

- [ ] Agent tool definitions (parse_location, run_pipeline, adjust_filters, describe_viewpoint)
- [ ] Claude Haiku integration via Anthropic API with tool use
- [ ] NL → filter config (agent interprets user requests into pipeline params)
- [ ] Result description generation (photographer-friendly narratives)
- [ ] Nano Banana 2 photorealistic enhancement
